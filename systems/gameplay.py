...
from systems.helpers import distance, hit_spikes, hit_cannonballs, hit_goblins, fell_out_of_world
...
def update_gameplay(state, dt):
    ...
    if state.merged is None:
        ...
        if state.merge_cooldown <= 0 and (state.p1_input.merge_pressed() or state.p2_input.merge_pressed()):
            if distance(state.player1, state.player2) <= MERGE_DISTANCE:
                state.effects.append(MergeEffect(state.player1.rect.center))

                mx = (state.player1.rect.centerx + state.player2.rect.centerx) // 2 - MERGED_W // 2
        ...
        if state.merge_cooldown <= 0 and state.merged.wants_split():
            ...
            state.player1.rect.midbottom = (state.merged.rect.centerx - split_dx, state.merged.rect.bottom)
            state.player2.rect.midbottom = (state.merged.rect.centerx + split_dx, state.merged.rect.bottom)
    ...
    if state.merged and state.level.finish:
        if state.merged.rect.colliderect(state.level.finish.rect):
            idx = state.level_classes.index(type(state.level))
            if idx + 1 > state.unlocked_levels:
                state.unlocked_levels = idx + 1
                state.save_data["unlocked_levels"] = state.unlocked_levels
                save_game(state.save_data)

            state.merged = None
            state.level_menu.open()
